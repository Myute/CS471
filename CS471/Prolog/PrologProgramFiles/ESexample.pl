...
% simple black dress is chosen if input matches brunette and slender.
choose(Hair, Skin, W, H, 'a simple black dress') :-
       brunette(Hair), slender(W, H).

% Laura Askley is chosen if input matches redish and fuller.
choose(Hair, Skin, W, H, 'Laura Ashley type') :-
       redish(Hair, Skin), fuller(W, H).

% pastel is chosen if input matches blonde and slender.
choose(Hair, Skin, W, H, 'a pastel dress') :-
       Hair = blonde, slender(W, H).

% *****  Add two more choose rules here
% sweatpants and a sweatshirt if input matches brunette and fuller.
choose(Hair, Skin, W, H, 'sweatpants and a sweatshirt') :-
       brunette(Hair), fuller(W, H).
% shorts and shirt if input matches slender.
choose(Hair,Skin,W,H,'shorts and a shirt.') :-
       slender(W,H).
